<template>
  <div>
    <h2 id="page-heading" data-cy="EnterpriseHeading">
      <span v-text="$t('iopApp.enterprise.home.title')" id="enterprise-heading">Enterprises</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('iopApp.enterprise.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EnterpriseCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('iopApp.enterprise.home.createLabel')"> Create a new Enterprise </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && enterprises && enterprises.length === 0">
      <span v-text="$t('iopApp.enterprise.home.notFound')">No enterprises found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('iopApp.enterprise.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('areaCode')">
              <span v-text="$t('iopApp.enterprise.areaCode')">Area Code</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'areaCode'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="iopApp.enterprise.delete.question" data-cy="enterpriseDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-enterprise-heading" v-text="$t('iopApp.enterprise.delete.question', { id: removeId })">
    ...
    <div v-show="enterprises && enterprises.length > 0">
